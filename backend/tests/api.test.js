...
process.env.ALLOWED_ORIGINS = 'https://smartlink-gules.vercel.app';
...
describe('GET /api/recent', () => {
  ...
  it('sends CORS headers for the deployed frontend origin', async () => {
    db.query.mockResolvedValueOnce({ rows: [mockUrl], rowCount: 1 });

    const res = await request(app)
      .get('/api/recent')
      .set('Origin', 'https://smartlink-gules.vercel.app');

    expect(res.statusCode).toBe(200);
    expect(res.headers['access-control-allow-origin']).toBe('https://smartlink-gules.vercel.app');
  });

  it('does not turn a disallowed CORS origin into a 500', async () => {
    db.query.mockResolvedValueOnce({ rows: [mockUrl], rowCount: 1 });
